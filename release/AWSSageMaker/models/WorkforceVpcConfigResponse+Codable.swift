...
extension SageMakerClientTypes.WorkforceVpcConfigResponse: Swift.Codable {
    ...
    public func encode(to encoder: Swift.Encoder) throws {
        ...
        if let securityGroupIds = securityGroupIds {
            ...
            for workforcesecuritygroupids0 in securityGroupIds {
                try securityGroupIdsContainer.encode(workforcesecuritygroupids0)
            }
        }
        if let subnets = subnets {
            var subnetsContainer = encodeContainer.nestedUnkeyedContainer(forKey: .subnets)
            for workforcesubnets0 in subnets {
                try subnetsContainer.encode(workforcesubnets0)
            }
        }
