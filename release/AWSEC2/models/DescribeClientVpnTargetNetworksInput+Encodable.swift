...
extension DescribeClientVpnTargetNetworksInput: Swift.Encodable {
    public func encode(to encoder: Swift.Encoder) throws {
        var container = encoder.container(keyedBy: ClientRuntime.Key.self)
        if let associationIds = associationIds {
            if !associationIds.isEmpty {
                for (index0, string0) in associationIds.enumerated() {
                    var associationIdsContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("AssociationIds.\(index0.advanced(by: 1))"))
                    try associationIdsContainer0.encode(string0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var associationIdsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("AssociationIds"))
        ...
        try container.encode("DescribeClientVpnTargetNetworks", forKey:ClientRuntime.Key("Action"))
