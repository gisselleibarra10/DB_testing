...
extension CreateIpamInput: Swift.Encodable {
    public func encode(to encoder: Swift.Encoder) throws {
        ...
        if let operatingRegions = operatingRegions {
            if !operatingRegions.isEmpty {
                for (index0, addipamoperatingregion0) in operatingRegions.enumerated() {
                    var operatingRegionsContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("OperatingRegion.\(index0.advanced(by: 1))"))
                    try operatingRegionsContainer0.encode(addipamoperatingregion0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var operatingRegionsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("OperatingRegion"))
                try operatingRegionsContainer.encode("", forKey: ClientRuntime.Key(""))
            }
        }
        ...
        try container.encode("CreateIpam", forKey:ClientRuntime.Key("Action"))
