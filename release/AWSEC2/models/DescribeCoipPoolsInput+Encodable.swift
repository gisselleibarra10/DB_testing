...
extension DescribeCoipPoolsInput: Swift.Encodable {
    public func encode(to encoder: Swift.Encoder) throws {
        ...
        if let poolIds = poolIds {
            if !poolIds.isEmpty {
                for (index0, ipv4poolcoipid0) in poolIds.enumerated() {
                    ...
                    try poolIdsContainer0.encode(ipv4poolcoipid0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var poolIdsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("PoolId"))
                try poolIdsContainer.encode("", forKey: ClientRuntime.Key(""))
            }
        }
        try container.encode("DescribeCoipPools", forKey:ClientRuntime.Key("Action"))
