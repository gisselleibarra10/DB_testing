...
extension DescribeVolumesModificationsInput: Swift.Encodable {
        ...
        if let volumeIds = volumeIds {
            if !volumeIds.isEmpty {
                for (index0, volumeid0) in volumeIds.enumerated() {
                    var volumeIdsContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("VolumeId.\(index0.advanced(by: 1))"))
                    try volumeIdsContainer0.encode(volumeid0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var volumeIdsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("VolumeId"))
                try volumeIdsContainer.encode("", forKey: ClientRuntime.Key(""))
            }
        }
        try container.encode("DescribeVolumesModifications", forKey:ClientRuntime.Key("Action"))
