...
extension EC2ClientTypes.ResourceStatementRequest: Swift.Codable {
        ...
        if let resourceTypes = resourceTypes {
            if !resourceTypes.isEmpty {
                for (index0, string0) in resourceTypes.enumerated() {
                    var resourceTypesContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("ResourceType.\(index0.advanced(by: 1))"))
                    try resourceTypesContainer0.encode(string0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var resourceTypesContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("ResourceType"))
