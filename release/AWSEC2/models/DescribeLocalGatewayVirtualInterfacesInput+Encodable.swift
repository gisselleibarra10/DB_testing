...
extension DescribeLocalGatewayVirtualInterfacesInput: Swift.Encodable {
    public func encode(to encoder: Swift.Encoder) throws {
        ...
        if let localGatewayVirtualInterfaceIds = localGatewayVirtualInterfaceIds {
            if !localGatewayVirtualInterfaceIds.isEmpty {
                for (index0, localgatewayvirtualinterfaceid0) in localGatewayVirtualInterfaceIds.enumerated() {
                    var localGatewayVirtualInterfaceIdsContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("LocalGatewayVirtualInterfaceId.\(index0.advanced(by: 1))"))
                    try localGatewayVirtualInterfaceIdsContainer0.encode(localgatewayvirtualinterfaceid0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var localGatewayVirtualInterfaceIdsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("LocalGatewayVirtualInterfaceId"))
        ...
        try container.encode("DescribeLocalGatewayVirtualInterfaces", forKey:ClientRuntime.Key("Action"))
