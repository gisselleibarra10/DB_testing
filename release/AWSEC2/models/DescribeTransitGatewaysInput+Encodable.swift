...
extension DescribeTransitGatewaysInput: Swift.Encodable {
        ...
        if let transitGatewayIds = transitGatewayIds {
            if !transitGatewayIds.isEmpty {
                for (index0, transitgatewayid0) in transitGatewayIds.enumerated() {
                    var transitGatewayIdsContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("TransitGatewayIds.\(index0.advanced(by: 1))"))
                    try transitGatewayIdsContainer0.encode(transitgatewayid0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var transitGatewayIdsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("TransitGatewayIds"))
                try transitGatewayIdsContainer.encode("", forKey: ClientRuntime.Key(""))
            }
        }
        try container.encode("DescribeTransitGateways", forKey:ClientRuntime.Key("Action"))
