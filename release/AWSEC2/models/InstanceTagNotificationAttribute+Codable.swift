...
extension EC2ClientTypes.InstanceTagNotificationAttribute: Swift.Codable {
        ...
        if let instanceTagKeys = instanceTagKeys {
            if !instanceTagKeys.isEmpty {
                for (index0, string0) in instanceTagKeys.enumerated() {
                    var instanceTagKeysContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("InstanceTagKeySet.\(index0.advanced(by: 1))"))
                    try instanceTagKeysContainer0.encode(string0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var instanceTagKeysContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("InstanceTagKeySet"))
                try instanceTagKeysContainer.encode("", forKey: ClientRuntime.Key(""))
            }
        }
    }
