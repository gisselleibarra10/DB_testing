...
struct ReleaseHostsOutputResponseBody: Swift.Equatable {
    let successful: [Swift.String]?
    let unsuccessful: [EC2ClientTypes.UnsuccessfulItem]?
}

extension ReleaseHostsOutputResponseBody: Swift.Decodable {
    ...
    public init (from decoder: Swift.Decoder) throws {
        ...
        if containerValues.contains(.successful) {
            ...
            if let successfulWrappedContainer = successfulWrappedContainer {
                let successfulContainer = try successfulWrappedContainer.decodeIfPresent([Swift.String].self, forKey: .member)
                var successfulBuffer:[Swift.String]? = nil
                if let successfulContainer = successfulContainer {
                    successfulBuffer = [Swift.String]()
