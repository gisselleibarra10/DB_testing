...
extension DescribeNetworkInterfacesInput: Swift.Encodable {
    public func encode(to encoder: Swift.Encoder) throws {
        ...
        if let networkInterfaceIds = networkInterfaceIds {
            if !networkInterfaceIds.isEmpty {
                for (index0, networkinterfaceid0) in networkInterfaceIds.enumerated() {
                    var networkInterfaceIdsContainer0 = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("NetworkInterfaceId.\(index0.advanced(by: 1))"))
                    try networkInterfaceIdsContainer0.encode(networkinterfaceid0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
                var networkInterfaceIdsContainer = container.nestedContainer(keyedBy: ClientRuntime.Key.self, forKey: ClientRuntime.Key("NetworkInterfaceId"))
        ...
        try container.encode("DescribeNetworkInterfaces", forKey:ClientRuntime.Key("Action"))
