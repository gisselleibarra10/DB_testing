...
extension EC2ClientTypes.LaunchPermissionModifications: Swift.Codable {
    ...
    public func encode(to encoder: Swift.Encoder) throws {
        ...
        if let add = add {
            if !add.isEmpty {
                for (index0, launchpermission0) in add.enumerated() {
                    ...
                    try addContainer0.encode(launchpermission0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
        ...
        if let remove = remove {
            if !remove.isEmpty {
                for (index0, launchpermission0) in remove.enumerated() {
                    ...
                    try removeContainer0.encode(launchpermission0, forKey: ClientRuntime.Key(""))
                }
            }
            else {
    ...
    public init (from decoder: Swift.Decoder) throws {
        ...
        if containerValues.contains(.add) {
            ...
            if let addWrappedContainer = addWrappedContainer {
                let addContainer = try addWrappedContainer.decodeIfPresent([EC2ClientTypes.LaunchPermission].self, forKey: .member)
                var addBuffer:[EC2ClientTypes.LaunchPermission]? = nil
                if let addContainer = addContainer {
                    addBuffer = [EC2ClientTypes.LaunchPermission]()
        ...
        if containerValues.contains(.remove) {
            ...
            if let removeWrappedContainer = removeWrappedContainer {
                let removeContainer = try removeWrappedContainer.decodeIfPresent([EC2ClientTypes.LaunchPermission].self, forKey: .member)
                var removeBuffer:[EC2ClientTypes.LaunchPermission]? = nil
                if let removeContainer = removeContainer {
                    removeBuffer = [EC2ClientTypes.LaunchPermission]()
